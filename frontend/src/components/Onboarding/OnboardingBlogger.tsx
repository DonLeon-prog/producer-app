...
const NICHES = [
  'Мода и стиль', 'Фитнес', 'Лайфстайл', 'Путешествия',
  'Бьюти', 'Кулинария', 'Личный бренд', 'Финансы', 'Юмор', 'Технологии',
];
...
export default function OnboardingBlogger() {
  ...
  const canProceed = () => {
    ...
    if (step === 4) return form.inspiration.trim().length > 0;
    if (step === 5) return form.goals.length > 0;
    return false;
  };

  const toggleMulti = (field: 'niches' | 'platforms', value: string) =>
    setForm(p => ({
      ...p,
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (isDev) {
        ...
        setUser({ ...DEV_USER_PATCH, user_type: 'blogger' } as any);
        setAnalysis('🎬 Dev-режим: AI-анализ заглушка.\n\nВ реальном приложении здесь будет персональный анализ ниши, конкуренты и конкретные советы с чего начать.');
        return;
      }
      const res = await client.post('/api/profile', { user_type: 'blogger', ...form });
      ...
      <div key={animKey} className="animate-slide-in">
        {step === 1 && (
          <>
            ...
            <p className="text-base text-white/50 mb-8">Имя или ник — как хочешь называться</p>
            <input
              autoFocus
              type="text"
              value={form.name}
        ...
        {step === 2 && (
          <>
            ...
            <div className="flex flex-wrap gap-3">
              {NICHES.map(n => (
                <Chip key={n} label={n} selected={form.niches.includes(n)} onToggle={() => toggleMulti('niches', n)} />
              ))}
            </div>
          </>
        )}
        ...
        {step === 4 && (
          <>
            ...
            <p className="text-base text-white/50 mb-8">Назови 1–3 блогера с похожим стилем</p>
            <input
              autoFocus
              type="text"
              value={form.inspiration}
              onChange={e => setForm(p => ({ ...p, inspiration: e.target.value }))}
              onKeyDown={e => e.key === 'Enter' && next()}
              placeholder="Например: Дудь, Варламов..."
              className="w-full h-14 bg-[#1A1A1A] rounded-2xl px-5 text-white text-lg placeholder-white/30 outline-none transition-all"
              style={{ border: '1.5px solid transparent' }}
              onFocus={e => (e.target.style.borderColor = '#7F77DD')}
              onBlur={e => (e.target.style.borderColor = 'transparent')}
        ...
        {step === 5 && (
          <>
            ...
            <div className="flex flex-wrap gap-3">
              {GOALS.map(g => (
                <Chip key={g} label={g} selected={form.goals === g} onToggle={() => setForm(p => ({ ...p, goals: g }))} />
              ))}
            </div>
          </>
        )}
